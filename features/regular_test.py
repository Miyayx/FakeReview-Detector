...
if __name__ == "__main__":
    fio = FileIO()
    clauses = fio.readFileToList("data/high_freq_clauses.dat")
    regulars = [
    ...
    #re.compile(ur"[\u4e00-\u9fa5]*[跟|和|与][照片|图片|描述|实物|店家描述|卖家描述]+[上的|中的]?[(一样)|(一致)|(没有差距)]"),
    #re.compile(ur"(?:几乎|也|基本上)?没有?色差"),
    ...
    for c in clauses:
        for regular in regulars:
            if re.match(regular,unicode(c,'utf-8')):
                print c
